...
const deletelogbylogger = async (ctx:RouterContext) => {
    try {
        ...
        if (!loggerCollection.map(c => c.loggerName).includes(loggerName)) {
            ...
            const deleteLog = await db.collection<Log>(loggerName).findOne({_id: new mongo.ObjectId(id)});
            if(!deleteLog){
                ctx.response.status = 404;
                ctx.response.body = {
                    msg: "Log not found"
            ...
            const log = await db.collection<Log>(loggerName).deleteOne({ _id: new mongo.ObjectId(id) });
